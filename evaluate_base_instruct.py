#!/usr/bin/env python3
"""
Evaluate base Qwen2.5-VL-3B-Instruct model on validation set.
This is the pretrained instruct model without any SFT or GRPO training.
"""
...
class BaseInstructEvaluator:
    def __init__(
        ...
        checkpoint_name: str = "base_instruct",
        ...
        """
        Initialize evaluator for base instruct model.

        Args:
            model_name: HuggingFace model name or path
            val_data_path: Path to validation parquet file
            output_dir: Base directory to save evaluation results
            project_name: Project name (folder level)
            experiment_name: Experiment name (folder level)
            checkpoint_name: Checkpoint name (folder level)
        ...
        print(f"Loading base instruct model: {model_name}")
        self.load_model()
    ...
    def save_results(self, eval_output: Dict[str, Any]):
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Base Instruct Model Evaluation Report\n")
            f.write("="*80 + "\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate base Qwen2.5-VL-3B-Instruct model")
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        "--checkpoint_name",
        type=str,
        default="base_instruct",
        help="Checkpoint name (folder level)"
    )
    parser.add_argument(
        "--max_samples",
        type=int,
    ...
    # Run evaluation
    evaluator = BaseInstructEvaluator(
