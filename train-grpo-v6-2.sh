...
SFT_MODEL=""
DATA_DIR="grpo_data"
OUTPUT_DIR="saves/qwen2_5vl-3b/grpo/adaptive_reasoning_9k_v6_2"
NUM_GPUS=8
ENGINE="vllm"
...
python3 -m verl.trainer.main_ppo \
    ...
    +reward_model.correct_reward=1.0 \
    +reward_model.incorrect_reward=0.0 \
    +reward_model.type1_format_bonus=$TYPE1_BONUS_INITIAL \
    +reward_model.type2_format_bonus=0.0 \
    +reward_model.type3_format_bonus=0.0 \
    +reward_model.type1_error_penalty=-0.5 \
    +reward_model.type2_error_penalty=0.0 \
    ...
    +reward_model.enable_bonus_decay=$ENABLE_DECAY \
    +reward_model.decay_strategy=$DECAY_STRATEGY \
    +reward_model.decay_start_step=$DECAY_START_STEP \
    +reward_model.decay_end_step=$DECAY_END_STEP \
    +reward_model.type1_bonus_min=$TYPE1_BONUS_MIN \
    +reward_model.decay_rate=$DECAY_RATE \
    +reward_model.normalize_answers=True \
    trainer.critic_warmup=0 \
    trainer.logger='["console","tensorboard","wandb"]' \
    trainer.project_name='verl_grpo_adaptive_reasoning_9k_v6' \
    trainer.experiment_name='qwen2_5vl_3b_adaptive_9k_v6_linear_2' \
